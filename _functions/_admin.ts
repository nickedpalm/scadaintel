...
/**
 * Notify the provider that their claim is in pending review.
 * Removes the "did that work?" anxiety after they submit a claim that
 * couldn't auto-verify via SMS. Sets expectations on timing + what
 * happens next.
 */
export async function notifyProviderClaimPending(
  env: Env,
  providerEmail: string,
  listingSlug: string,
): Promise<void> {
  ...
  const siteDomain = site.domain;
  const fromEmail = site.fromEmail;
  const replyTo = `contact@${siteDomain}`;
...
<div style="max-width:480px;margin:40px auto;background:#fff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
  ...
  <div style="padding:32px">
    <h2 style="color:#1a1a1a;font-size:18px;margin:0 0 12px">Your claim is under review</h2>
    <p style="color:#6b7280;font-size:15px;line-height:1.6;margin:0 0 16px">
      Thanks for claiming <strong>${esc(listingSlug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()))}</strong>. We couldn't auto-verify you right now, so a human is going to take a quick look.
    </p>
    <p style="color:#6b7280;font-size:15px;line-height:1.6;margin:0 0 16px">
      <strong>Typical turnaround:</strong> within 24 hours (usually much faster).
    </p>
    <p style="color:#6b7280;font-size:15px;line-height:1.6;margin:0 0 16px">
      Once approved, you'll get an email at this address and can log in to edit your profile, upload photos, respond to quote requests, and manage your listing.
    </p>
  ...
  <div style="border-top:1px solid #f3f4f6;padding:20px 32px">
    <p style="color:#d1d5db;font-size:11px;margin:0;line-height:1.5">
      ${siteName}${site.address ? ` &middot; ${site.address}` : ''}
    </p>
  </div>
</div>
</body>
  ...
  try {
    // Ensure subscriber exists for Listmonk tx
    await fetch(`${site.listmonkUrl}/api/subscribers`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': auth },
      body: JSON.stringify({ email: providerEmail, name: '', status: 'enabled' }),
    });
    ...
    const resp = await fetch(`${site.listmonkUrl}/api/tx`, {
      ...
      headers: { 'Content-Type': 'application/json', 'Authorization': auth },
      body: JSON.stringify({
        subscriber_email: providerEmail,
        template_id: site.passthroughTemplateId,
        subject: `Your ${siteName} claim is under review`,
        data: { body: htmlBody },
        content_type: 'html',
        messenger: 'email',
        from_email: fromEmail,
    ...
    if (!resp.ok) {
      const text = await resp.text();
      console.error(`Provider claim-pending email failed: ${resp.status} ${text}`);
    }
  } catch (err) {
    console.error('Provider claim-pending notification error:', err);
  }
}
