...
import { notifyAdminPendingClaim, notifyProviderClaimPending } from '../../_admin';
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  // Notify the provider — sets expectations ("under review, usually < 24h")
  // so they don't sit wondering whether the claim went through. Non-blocking:
  // even if this fails, the claim is still recorded and the admin was
  // notified.
  try {
    await notifyProviderClaimPending(env, provider.email, slug);
  } catch (err) {
    console.error('Provider claim-pending notification failed:', err);
  }
