...
<div style="max-width:480px;margin:40px auto;background:#fff;border-radius:12px;border:1px solid #e5e7eb;overflow:hidden">
  ...
  try {
    ...
    // Subject varies by intent: generic-login vs listing-claim.
    // Claim flow is the higher-value path (we know who they are —
    // they're claiming a specific business), so signal that in the subject.
    const subject = listingSlug
      ? `Claim your listing on ${siteName}`
      : `Your ${siteName} login link`;

    // Reply-To points to the vertical's real inbox (CF Email Routing
    // forwards contact@<domain> → system.ops.central@gmail.com). Without
    // this, replies hit newsletter@<domain> which bounces. Set at the
    // SMTP header level via Listmonk's headers array.
    const replyTo = `contact@${domain}`;
